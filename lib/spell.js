  ...
  constructor (languages) {
    ...
    if (languages && languages.length > 0) {
      ...
      for (const language of languages) {
        let rank = 0
        let dict
        for (const entry of dictionaries.entries()) {
          const newRank = LanguageRange.matches(entry[0], language)
          console.log(`${entry} -> ${newRank}`)
          if (newRank > rank) {
            rank = newRank
            dict = entry[1]
          }
        ...
        if (dict) {
          ...
          atom.notifications.addWarning('Missing Dictionary', { detail: `Unable to find a dictionary for language ${language}.` })
        }
      }
      if (d.length > 0) {
        switch (this.ispell) {
