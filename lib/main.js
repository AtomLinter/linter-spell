...
const Main = {
  ...
  isIgnored (scopeDescriptor) {
    ...
    let i = path.length
    while (i--) {
      for (const [scope, vote] of this.scopes) {
  ...
  scopeVote (scope, vote) {
    if (this.scopes.has(scope)) {
      this.scopes.set(scope, this.scopes.get(scope) + vote)
    } else {
      this.scopes.set(scope, vote)
    }
  },

  grammarScopeVotes (grammar, falseVote, trueVote) {
    if (grammar.scopes) {
      _.forEach(grammar.scopes, (value, key) => this.scopeVote(key, value ? trueVote : falseVote))
    } else {
