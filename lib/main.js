            ...
            while (filtered.ranges[0].containsPoint(current)) {
              ...
              if (global.grammarManager.isIgnored(scopeDescriptor)) {
                if (current === filtered.ranges[0].start) {
                  ...
                  let embeddedGrammar = global.grammarManager.getEmbeddedGrammar(scopeDescriptor)
                  if (embeddedGrammar) {
                    if (embeddedRanges.has(embeddedGrammar)) {
                      embeddedRanges.get(embeddedGrammar).push(filtered.ranges.shift())
                    } else {
                      embeddedRanges.set(embeddedGrammar, [filtered.ranges.shift()])
                    }
                  }
