...
const Main = {
  ...
  consumeScopes (scopes) {
    ...
    scopes.ignore([
      '.source.gfm .link .markup.underline.link.gfm',
  ...
  consumeGrammar (grammars) {
    ...
    for (const grammar of grammars) {
      ...
      if (grammar.ignoredScopes) {
        for (const scope of grammar.ignoredScopes) {
          if (scope in this.ignoredScopes) {
            this.ignoredScopes[scope]++
          } else {
            this.ignoredScopes[scope] = 1
          }
        }
      }
    }
    return new Disposable(() => {
      for (const grammar of grammars) {
        ...
        if (grammar.ignoredScopes) {
          for (const scope of grammar.ignoredScopes) {
            if (--this.ignoredScopes[scope] === 0) {
              delete this.ignoredScopes[scope]
            }
          }
        }
      }
    ...
    return {
      ...
      lint: async (textEditor, scopeName) => {
        ...
        if (grammar) {
          ...
          let rm = []
          let current = textEditor.getBuffer().getRange()
          while (!current.isEmpty()) {
            let scope = _.map(textEditor.scopeDescriptorForBufferPosition(current.start).getScopesArray(), part => part[0] === '.' ? part : ('.' + part)).join(' ')
