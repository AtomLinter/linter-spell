...
//      getDictionaries: textEditor => [],
      ...
      getDictionaries: textEditor => {
        const dictionaryPattern = /^%\s*!TEX\s+spellcheck\s*=\s*(.*)$/im
        const dictionarySeparatorPattern = /(?:\s,)+/
        const match = textEditor.getBuffer().getText().match(dictionaryPattern)
        return match ? match[1].split(dictionarySeparatorPattern) : []
      },
      getRanges: textEditor => [textEditor.getBuffer().getRange()]
    }]
  }
  ...
  getChecker (dictionaries) {
    ...
    if (!this.checkers[id]) {
      this.checkers[id] = new Hunspell(name, dictionaries)
    }
