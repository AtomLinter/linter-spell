      ...
      lint: async (textEditor, scopeName) => {
        ...
        if (grammar) {
          ...
          if (checker !== null) {
            ...
            while (filtered.ranges[0].containsPoint(current)) {
              ...
              if (global.grammarManager.isIgnored(scopeDescriptor)) {
                if (current === filtered.ranges[0].start) {
                  ...
                  let embedded = _.findLast(scopeDescriptor.getScopesArray(), scope => scope.match(/^embedded\./))
                  if (embedded) {
                    let embeddedScope = embedded.replace(/^embedded\./, '')
                    if (embeddedRanges.has(embeddedScope)) {
                      embeddedRanges.get(embeddedScope).push(filtered.ranges.shift())
                    } else {
                      embeddedRanges.set(embeddedScope, [filtered.ranges.shift()])
                    }
                  }
            ...
            console.log(embeddedRanges)
            for (const entry of embeddedRanges.entries()) {
              console.log(entry)
              const embeddedGrammar = global.grammarManager.getGrammar(entry[0])
              console.log(embeddedGrammar)
              if (embeddedGrammar && embeddedGrammar.filterRanges) {
                const embeddedFiltered = embeddedGrammar.filterRanges(textEditor, entry[1])
                console.log(embeddedFiltered)
            ...
            _.remove(filtered.ranges, range => range.isEmpty())
