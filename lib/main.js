        ...
        if (grammar) {
          ...
          if (grammar._ignoredPattern !== null) {
            ...
            while (ranges[ranges.length - 1].containsPoint(current)) {
              let next = (textEditor.getBuffer().lineLengthForRow(current.row) === current.column) ? new Point(current.row + 1, 0) : new Point(current.row, current.column + 1)
              if (grammar._ignoredPattern.test(getScopePath(textEditor.scopeDescriptorForBufferPosition(current)))) {
