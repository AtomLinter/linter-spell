...
class Providers {
  constructor(managers) {
    const { grammarManager, languageManager, dictionaryManager } = managers
  ...
  provideLinter () {
    return {
      ...
      lint: async (textEditor, scopeName) => {
        let messages = []
        this.grammarManager.updateLanguage(textEditor, this.languageManager)
        const grammar = this.grammarManager.getGrammar(textEditor)
        if (grammar) {
          const filePath = textEditor.getPath()
          const languages = this.languageManager.getLanguages(textEditor)
          let filtered = {
          ...
          while (filtered.ranges[0].containsPoint(current)) {
            ...
            const isIgnored = this.grammarManager.isIgnored(scopeDescriptor)
            let embeddedGrammar = isIgnored ? null : this.grammarManager.getEmbeddedGrammar(scopeDescriptor)
          ...
          for (let checkRange of filtered.ranges) {
            const misspellings = await this.dictionaryManager.checkRange(textEditor, languages, checkRange)
            for (const misspelling of misspellings) {
              let word = textEditor.getTextInBufferRange(misspelling.range)
              let solutions = []
              if (misspelling.suggestions) {
                solutions = _.map(misspelling.suggestions, (suggestion, index) => ({
