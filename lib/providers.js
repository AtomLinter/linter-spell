    ...
    lint: async (textEditor, scopeName) => {
      ...
      if (grammar) {
        ...
        for (let checkRange of filtered.ranges) {
          for (const misspelling of await global.dictionaryManager.checkRange(textEditor, languages, checkRange)) {
            console.log(misspelling)
            let word = textEditor.getTextInBufferRange(textEditor.getTextInBufferRange(misspelling.range))
            let intentions = []
            if (misspelling.suggestions) {
              intentions = _.map(misspelling.suggestions,
              (suggestion, index) => ({
                icon: 'light-bulb',
                title: `Change to "${suggestion}"`,
                selected: () => textEditor.setTextInBufferRange(misspelling.range, suggestion)
              }))
            }
            ...
            messages.push({
              type: 'Warning',
              text: `${word} -> ${misspelling.suggestions.join(', ')}`,
