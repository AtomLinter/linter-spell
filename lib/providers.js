...
import { wordbreak } from 'unicodeJS'
    ...
    lint: async (textEditor, scopeName) => {
      ...
      if (grammar) {
        ...
        for (let checkRange of filtered.ranges) {
          const text = textEditor.getTextInBufferRange(checkRange)
          for (let wordRange of foo(checkRange)) {
            const result = await global.dictionaryManager.checkWord(textEditor, languages, wordRange)
            if (!result.isWord) {
              let word = textEditor.getTextInBufferRange(wordRange)
              const intentions = []
              if (result.suggestions) {
                intentions = _.map(result.suggestions,
                (suggestion, index) => ({
                  icon: 'light-bulb',
                  title: `Change to "${suggestion}"`,
                  selected: () => textEditor.setTextInBufferRange(wordRange, suggestion)
                }))
              }
              if (result.actions) {
                intentions = _.concat(intentions,
                  _.map(result.actions, (action, index) => ({
                    icon: 'check',
                    title: action.title,
              ...
              _.forEach(intentions, (intention, index) => intention.priority = 100 + intentions.length - index)
              messages.push({
                type: 'Warning',
                text: `${word} -> ${result.suggestions.join(', ')}`,
                range: wordRange,
