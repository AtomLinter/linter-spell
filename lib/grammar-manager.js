...
export default class GrammarManager extends Disposable {
  ...
  getGrammar (textEditor) {
    const scope = _.isString(textEditor) ? textEditor : textEditor.getGrammar().scopeName
  ...
  isIgnored (scopeDescriptor) {
    ...
    let i = path.length
    while (i--) {
      const scope = path[i].replace(/^embedded\./, '')
      if (this.checkedScopes.has(scope)) {
        return this.checkedScopes.get(scope) < 0
      }
    }
    return true
  }
