...
import { Range, Disposable, Emitter, CompositeDisposable } from 'atom'
  ...
  provideDictionary () {
    return [{
      ...
      checkWord: (textEditor, languages, range) => {
        ...
        const misspellings = checker.getMisspellings(textEditor, range)
...
class Spell extends Disposable {
  constructor (languages) {
    ...
    if (languages && languages.length > 0) {
      ...
      let d = global.languageManager.resolveDictionaries(languages)
      if (d.length > 0) {
        switch (this.ispell) {
