...
class LanguageStatusView extends HTMLDivElement {
  initialize (statusBar, languageManager) {
    ...
    this.disposables.add(atom.tooltips.add(this.languageLink, {title: () => {
      ...
      if (textEditor != null) {
        return _.map(
          this.languageManager.getLanguages(textEditor),
  ...
  handleEvents () {
    this.disposables.add(this.languageManager.onDidChangeLanguages(() => {
  ...
  updateLanguageText () {
    ...
    let languages
    if (textEditor != null) {
      languages = this.languageManager.getLanguages(textEditor)
    }
    if (languages) {
